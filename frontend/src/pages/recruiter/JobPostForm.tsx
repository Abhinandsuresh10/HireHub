...
const JobPostForm = () => {
  ...
  const [rolesCategories, setRolesCategories] = useState<JobRoleCategories[]>();
  const [selectedCategoryId, setSelectedCategoryId] = useState<string>("");

  const selectedCategory = rolesCategories?.find(cat => cat._id === selectedCategoryId);

  const [skills, setSkills] = useState([""]);
  const [selectedSkills, setSelectedSkills] = useState<SkillsCategories[]>([]);
  const [selectedSkillCategoryId, setSelectedSkillCategoryId] = useState<string>("");

  const selectedSkillCategory = selectedSkills.find(cat => cat._id === selectedSkillCategoryId);

  useEffect(() => {
    const fetchJobRoles = async () => {
      const response = await getJobRoles();
      if (response?.data) {
        setRolesCategories(response.data.jobRoles)
      }
    }
    fetchJobRoles()
  }, []);
