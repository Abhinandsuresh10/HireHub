...
import LoginImage from '../../assets/login_image_2.avif'
...
import GoogleLoginBtn from '../../components/user/GoogleLoginBtn'
...
const Login = () => {
 ...
 const { register: registerReset, handleSubmit:handleResetPassword, formState: {errors: resetErrors}} = useForm<ResetFormData>({
...
const onResetSubmit = async(data: ResetFormData) => {
    ...
    try {
        const response = await UserAPI.post('/setNewPassword',{password: password, email: forgotEmail});
        if(response.data) {
            setStep(1)
...
const handleForgot = async() => {
   try {
    const response = await UserAPI.post('/forgotPassword', {email: forgotEmail});
    if(response.data) {
        setStep(3);
        toast.success(response.data?.message);
    }
...
const handleOtpForgot = async () => {
    ...
    try {
        const response = await UserAPI.post('/verifyForgotOtp', {email: forgotEmail,otp: otpInput});
        if(response.data) {
            setStep(4);
            toast.success(response.data?.message);
        }
  ...
  return (
    ...
    {/* Left section */}
     { step === 1 && ( <div className="md:w-1/2 p-6 flex flex-col justify-center">
        <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">Login to HireHub</h2>
        ...
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <p className='text-gray-700'>Don't have an account ?<Link to='/register' className='text-black'> register</Link></p>
