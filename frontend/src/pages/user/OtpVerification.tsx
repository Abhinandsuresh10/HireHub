...
const OtpVerification = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await UserAPI.post('/verifyOTP', { otp: input });
      ...
      if (axios.isAxiosError(error) && error.response) {
        const errorMessage = error.response.data?.error || error.response.statusText || 'Invalid OTP';
  ...
  const handleResend = async () => {
    const response = await UserAPI.post('/resentOtp');
