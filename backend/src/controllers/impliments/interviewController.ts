...
export class interviewController {
  ...
  public getInterviews = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const interviewType = req.query.interviewType as string;
      const { data, total } = await this.service.getInterviews(recruiterId, page, limit, interviewType);
  ...
  public resheduleInterview = async (req: Request, res: Response): Promise<void> => {
    try {
       const id = req.params.id as string;
       const date = req.body.data.date as Date;
       const time = req.body.data.time as string;
      console.log(id, date, time)
       await this.service.resheduleInterview(id, date, time);
       res.status(HttpStatus.OK).json({ message: HttpResponse.INTERVIEW_RESHEDULED})
    } catch (error) {
      if (error instanceof Error) {
        res.status(HttpStatus.BAD_REQUEST).json({ error: error.message });
      } else {
  ...
  public getInterview = async (req: Request, res: Response): Promise<void> => {
    try {
      const id = req.params.id as string;
