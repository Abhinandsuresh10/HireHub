     ...
     public register = async (req: Request, res: Response): Promise<void> => {
      ...
      public verifyOTP = async (req: Request, res:Response): Promise<void> => {
        try {
          ...
          res.status(201).json({message: HttpResponse.RECRUITER_CREATE_SUCCESS});
        ...
        res.status(200).json({
          ...
          if (error instanceof Error) {
            if (error.message === HttpResponse.RECRUITER_NOT_FOUND) {
                res.status(404).json({ error: HttpResponse.RECRUITER_NOT_FOUND });
                return;
                ...
                res.status(403).json({ error: HttpResponse.RECRUITER_BLOCK });
                return;
             }
           }
          res.status(500).json({ error: HttpResponse.SERVER_ERROR });
      ...
      public googleLogin = async (req: Request, res:Response): Promise<void> => {
        ...
        res.status(200).json({message: HttpResponse.RECRUITER_LOGIN_SUCCESS, data})
        } catch (error) {
          if (error instanceof Error) {
            res.status(400).json({ error: error.message }); 
        } else {
      ...
      public editRecruiter = async (req: Request, res: Response): Promise<void> => {
        try {
                ...
                const recruiterData = {
                 ...
                 res.status(201).json({message: HttpResponse.RECRUITER_EDIT_SUCCESS, recruiter})
        } catch (error) {
        if (error instanceof Error) {
            res.status(400).json({ error: error.message }); 
           } else {
