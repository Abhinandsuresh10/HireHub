...
export class adminController {
  ...
  public getDashboardStats = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(HttpStatus.OK).json({ message: HttpResponse.ADMIN_DASHBOARD_GET_SUCCESS, stats });
    } catch (error) {
      if (error instanceof Error) {
        res.status(HttpStatus.BAD_REQUEST).json({ error: error.message });
      } else {
  ...
  public getDashboardBars = async (req: Request, res: Response): Promise<void> => {
    try {
      const { applications, interviews } = await this.service.getDashboardBars();
      res.status(HttpStatus.OK).json({ message: HttpResponse.ADMIN_DASHBOARD_GET_SUCCESS, applications, interviews })
    } catch (error) {
      if (error instanceof Error) {
        res.status(HttpStatus.BAD_REQUEST).json({ error: error.message });
      } else {
  ...
  public getDashboardLineData = async (req: Request, res: Response): Promise<void> => {
    try {
      const { lineData } = await this.service.getDashboardListData();
      res.status(HttpStatus.OK).json({ message: HttpResponse.ADMIN_DASHBOARD_GET_SUCCESS, lineData })
    } catch (error) {
      if (error instanceof Error) {
        res.status(HttpStatus.BAD_REQUEST).json({ error: error.message });
      } else {
