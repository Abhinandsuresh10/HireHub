...
export class applicationController {
  ...
  public isApplied = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const status = await this.service.isApplied(userId, jobId)
  ...
  public rejectApplication = async (req: Request, res: Response): Promise<void> => {
    try {
      const id = req.query.id as string;
      const application = await this.service.rejectApplication(id);
      res.status(HttpStatus.OK).json({ message: HttpResponse.APPLICANTS_GET_SUCCESS, application })
    } catch (error) {
      if (error instanceof Error) {
        res.status(HttpStatus.BAD_REQUEST).json({ error: error.message });
  ...
  public getAppliedApplication = async (req: Request, res: Response): Promise<void> => {
    try {

      const userId = req.query.userId as string;
      const jobId = req.query.jobId as string;

      const application = await this.service.getAppliedApplication(userId, jobId);
      res.status(HttpStatus.OK).json({ message: HttpResponse.APPLICANTS_GET_SUCCESS, application })
  ...
  public declineMail = async (req: Request, res: Response): Promise<void> => {
    try {
      const { reason, name, email, jobId } = req.body;
      const jobRole = await this.service.getJobRole(jobId);

      await this.service.declineEmail(reason, name, email, jobRole);

      res.status(HttpStatus.OK).json({ message: HttpResponse.DECLINE_MAIL_SENT })
    } catch (error) {
      if (error instanceof Error) {
        res.status(HttpStatus.BAD_REQUEST).json({ error: error.message });
      } else {
  ...
  public hireInterviewe = async (req: Request, res: Response): Promise<void> => {
    try {
      const id = req.query.id as string;
      await this.service.hireInterviewe(id);
      res.status(HttpStatus.OK).json({ message: HttpResponse.APPLICATION_HIRED_SUCCESS })
    } catch (error) {
      if (error instanceof Error) {
        res.status(HttpStatus.BAD_REQUEST).json({ error: error.message });
      } else {
